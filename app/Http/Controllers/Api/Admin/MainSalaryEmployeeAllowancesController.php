...
class MainSalaryEmployeeAllowancesController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        try {
            ...
            $query = Main_salary_employee_allowance::with([
            ...
            $types = Allowance::where('com_code', $com_code)->where('active', 1)->select('id', 'name')->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $allowance = Main_salary_employee_allowance::create([
            ...
            return response()->json(['status' => true, 'message' => 'تم إضافة البدل بنجاح']);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $allowance = Main_salary_employee_allowance::where(['id' => $id, 'com_code' => $com_code, 'is_archived' => 0])->first();
            if (!$allowance) {
            ...
            DB::beginTransaction();
            $old_main_salary_employee_id = $allowance->main_salary_employee_id;
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            $financeMonth_data = Finance_months_periods::with('month')->where(['com_code' => $com_code, 'id' => $allowance->finance_month_periods_id, 'is_open' => 1])->first();
            if (!$financeMonth_data) {
            ...
            $mainSalaryEmployee_data = Main_salary_employee::where([
                'com_code' => $com_code, 
                'id' => $allowance->main_salary_employee_id, 
                'is_archived' => 0
            ])->first();
