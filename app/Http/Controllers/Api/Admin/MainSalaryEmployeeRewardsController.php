...
class MainSalaryEmployeeRewardsController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        try {
            ...
            $query = Main_salary_employee_reward::with([
            ...
            $types = Additional_sal_type::where('com_code', $com_code)->where('active', 1)->select('id', 'name')->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $reward = Main_salary_employee_reward::create([
                ...
                'additions_type_id' => $request->additions_type_id,
                'total' => $request->total,
                'notes' => $request->notes,
                'com_code' => $com_code,
                'added_by' => $user->id,
            ...
            return response()->json(['status' => true, 'message' => 'تم إضافة المكافأة بنجاح']);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $reward = Main_salary_employee_reward::where(['id' => $id, 'com_code' => $com_code, 'is_archived' => 0])->first();
            if (!$reward) {
            ...
            DB::beginTransaction();
            $old_main_salary_employee_id = $reward->main_salary_employee_id;
            ...
            return response()->json(['status' => true, 'message' => 'تم تعديل المكافأة بنجاح']);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            $financeMonth_data = Finance_months_periods::with('month')->where(['com_code' => $com_code, 'id' => $reward->finance_month_periods_id, 'is_open' => 1])->first();
            if (!$financeMonth_data) {
            ...
            $mainSalaryEmployee_data = Main_salary_employee::where([
                'com_code' => $com_code, 
                'id' => $reward->main_salary_employee_id, 
                'is_archived' => 0
            ])->first();
            ...
            return response()->json(['status' => true, 'message' => 'تم حذف المكافأة بنجاح']);
        } catch (\Exception $e) {
            DB::rollBack();
